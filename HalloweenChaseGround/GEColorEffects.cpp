...
// strobe effect
void GEColorEffects::strobeGround(color_t color, int* positions, int bulb_count)
{
  int min_num_strobes = 30;
  int max_num_strobes = 60;
  int min_delay = 30;
  int max_delay = 60;
...
for (int j = 0; j < bulb_count; j++)
    fill_color(positions[j], 1, DEFAULT_INTENSITY, color);
    delay(tdelay);
  
    // strobe off
for (int j = 0; j < bulb_count; j++)
    fill_color(positions[j], 1, 0, 0);
    delay(tdelay);
  }
}
...
// sets entire string to random color per bulb 
void GEColorEffects::set_string_colors(color_t color1, color_t color2, color_t color3)
{
  int i;
  color_t colors[3]={color1,color2,color3};
  for (i = 0; i < lightCount; i++) {
    set_color((uint8_t)i, DEFAULT_INTENSITY, colors[random(0,3)]);
  }
}

// strobe 2-5 random bulbs
void GEColorEffects::strobeToColor(color_t color1, color_t color2, color_t color3)
{
    color_t colors[3]={color1,color2,color3};
//pick 2-5 random non-linear bulbs in string
  int n = random(2,5);
  int positions[5];
  int i; 
  for (i = 0; i < n; i++)
    positions[i]=random(0,lightCount);

//strobe those 2-5 bulbs for random length of time, random rate
  strobeGround(COLOR_WHITE,positions,n);

// set those 2-5 bulbs back to PGO
  for (i = 0; i < n; i++)
     set_color(positions[i],DEFAULT_INTENSITY, colors[random(0,3)]);
}
